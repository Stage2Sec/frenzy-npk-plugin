...
export class NpkPricing {
    ...
    public getSpotPriceHistory(instanceType: string, forceRegion?: string) {
        ...
        switch (forceRegion) {
            case "us-west-1":
                regions.push(this.ec2w1);
                break;

            case "us-west-2":
                regions.push(this.ec2w2);
                break;

            case "us-east-1":
                regions.push(this.ec2e1);
                break;

            case "us-east-2":
                // regions.push(this.ec2e2);
        ...
        var promises: Array<Promise<EC2.DescribeSpotPriceHistoryResult>> = [];

        regions.forEach((e) => {
            promises.push(new Promise((resolve, reject) => {
                let region = e.config.region || ""
                if (typeof this.spotPriceHistory[region][instanceType] != "undefined") {
                    return resolve(this.spotPriceHistory[region][instanceType]);
                }
                e.describeSpotPriceHistory({
                    StartTime: new Date(Math.round(Date.now() / 1000) * 1000),
                    EndTime: new Date(Math.round(Date.now() / 1000) * 1000),
                    InstanceTypes: [
                        instanceType
                    ],
                    ProductDescriptions: [
                    ...
                    this.spotPriceHistory[region][instanceType] = data;
                    resolve(data);
                });
            }));
        });
        ...
        return Promise.all(promises).then((data) => {
            ...
            Object.keys(data).forEach((e) => {
                Object.keys(data[e].SpotPriceHistory).forEach((i) => {
                    let item = data[e].SpotPriceHistory[i];

                    if (results.price == null || parseFloat(item.SpotPrice) < results.price) {
                        results = {
    ...
    public getAttributes(attr: any) {
        return new Promise((resolve, reject) => {
            this.pricing.getAttributeValues({
    ...
    public async getLowestPrice(list: any, forceRegion?: string) {
        var promises: Array<Promise<any>> = [];

        Object.keys(list).forEach(e => {
            promises.push(this.getSpotPriceHistory(e, forceRegion));
        });
        ...
        Object.keys(data).forEach((i) => {
            ...
            if (forceRegion && data[i].price == null) {
                return false;
            }

            // TODO: Switch '<' at 551:115 to '>' to pick bigger instances.
            // Is better if cheaper or same price with more GPUs
            if (result.price == null || (data[i].price / list[data[i].instanceType]) < result.price ||
    ...
    public async getInstancePrices(forceRegion?: string) {
        let cheapest_g3 = this.getLowestPrice({
            // "g3s.xlarge": 1,
            "g3.4xlarge": 1,
            "g3.8xlarge": 2,
            "g3.16xlarge": 4
        ...
        let cheapest_p2 = this.getLowestPrice({
            "p2.xlarge": 1,
            "p2.8xlarge": 8,
            "p2.16xlarge": 16
        }, forceRegion);

        let cheapest_p3 = this.getLowestPrice({
            "p3.2xlarge": 1,
            "p3.8xlarge": 4,
            "p3.16xlarge": 8
        }, forceRegion);
        ...
        return {
            idealG3Instance: {
                type: data[0].cheapestType,
                price: data[0].price,
                az: data[0].cheapestRegion,
                gpus: this.gpus[data[0].cheapestType]
            },
    
            idealP2Instance: {
                type: data[1].cheapestType,
                price: data[1].price,
                az: data[1].cheapestRegion,
                gpus: this.gpus[data[1].cheapestType]
            },
    
            idealP3Instance: {
                type: data[2].cheapestType,
                price: data[2].price,
                az: data[2].cheapestRegion,
                gpus: this.gpus[data[2].cheapestType]
            },
        }
    }
    ...
    public async getHashPricing(hashType: string, forceRegion?: string): Promise<any> {
        ...
        let instancePrices = await this.getInstancePrices(forceRegion);
        ["g3", "p2", "p3"].forEach((e) => {
            if (isFalsy(hashType)) {
                instances[e] = empty;
            }
...
export const npkPricing = new NpkPricing()